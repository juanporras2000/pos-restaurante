    ...
    return (
        <Modal abierto={abierto} onCerrar={onCerrar}>
                <div className="p-5 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
                    <h2 className="text-base font-semibold text-gray-900 dark:text-gray-100">Ajustar stock — {insumo.nombre}</h2>
                ...
                <div className="p-5 space-y-4">
                        ...
                        <div className="flex gap-2">
                            {['entrada', 'ajuste'].map((t) => (
                                <button key={t} type="button" onClick={() => setTipo(t)}
                                    className={`flex-1 py-2 rounded-lg text-sm font-medium border transition-colors ${tipo === t ? 'bg-blue-600 text-white border-blue-600' : 'border-gray-300 text-gray-700 dark:text-gray-300 hover:bg-gray-50'}`}>
                    ...
                    <div>
                        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                            {tipo === 'entrada' ? 'Cantidad a agregar' : 'Nuevo stock'} <span className="text-gray-400 dark:text-gray-500">({insumo.unidad_medida})</span>
                        </label>
                        <input
                            autoFocus
                ...
                <div className="px-5 pb-5 flex justify-end gap-3">
                    <button type="button" onClick={onCerrar} className="px-4 py-2 border border-gray-300 text-gray-700 dark:text-gray-300 rounded-lg hover:bg-gray-50 text-sm">Cancelar</button>
